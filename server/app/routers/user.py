...
async def register(email: EmailStr = Form(...), password: str = Form(..., min_length=6)):
    ...
    user_dict = {
        'uid': uuid4(),
        'avatar': 'static/users/default_avatar.png',
        'email': email,
        ...
        content={
            'access_token': auth.generate_access_token({
                **json_compatible_user,
                ...
                'is_staff': False,
                'is_superuser': False,
                'avatar_url': f'{comm.generate_base_url()}/static/users/default_avatar.png',
                'mobile_hash': ''
            }),
            'token_type': 'bearer'
        },
...
async def update_user_info(username: str = Form(...), avatar: HttpUrl = Form(...),
    ...
    if not avatar_path.startswith('static/users/'):
