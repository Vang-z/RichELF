...
export const Moments: React.FC<SizeProps> = ({size}) => {
  ...
  const loadingMore = useCallback(() => {
    ...
    if (loadingMore) {
      ...
      if ((offsetTop - scrollTop <= viewPortHeight + 960) && momentsContent && momentsContent.nextPage && !loading) {
        dispatch(getMoments(momentsContent.nextPage))
      }
    }
  ...
  return <Box id={`activity`} className={styles.Container}>
    ...
    <Box className={styles.ActivityBorder}>
      ...
      {
        momentsContent && momentsContent.data.map((timeline: any) => {
